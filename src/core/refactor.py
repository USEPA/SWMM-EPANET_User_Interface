...
if __name__ == '__main__':
    ...
    for root, subdirs, files in os.walk(start_dir):
        ...
        for filename in files:
            ...
            rewrite_filename = os.path.join(root, file_base).replace(original_subpath, refactor_subpath) + ".py"
            ...
            if ext == ".py" and not file_base.startswith(inp_prefix):
                ...
                if reader_old_method in file_contents or writer_old_method in file_contents:
                            ...
                            if "staticmethod" in line:
                                ...
                                if line is not None:
                                    ...
                                    if " def " in line:
                                        ...
                                        new_line = line.replace(orig_class_name + ".field_format",
                                                                writer_class_name + ".field_format")
                                        new_line = new_line.replace("Temperature.field_format",
                                                                    "TemperatureWriter.field_format")
                                        new_line = new_line.replace("self.field_format",
                                                                    writer_class_name + ".field_format")
                                    ...
                                    if line is not None:
                                        ...
                                        if in_read_method:
                                            if orig_class_name:
                                                ...
                                                new_line = new_line.replace("Section.__init__(" + class_var_name + ")",
                                                                            class_var_name + " = " + orig_class_name + "()")
                                            # if class_var_name and reader_class_name:
                                            #     new_line = new_line.replace(class_var_name + ".get_attr_name_value(",
                                            #                                 reader_class_name + ".get_attr_name_value(" +
                                            #                                 class_var_name + ", ")
                                        ...
                                        if rewrite_line is not None and not in_read_method and not in_write_method:
                                            ...
                                            rewrite_line = rewrite_line.replace("import InputFile, Section",
                                                                                "import Section")
                                            rewrite_line = rewrite_line.replace("core.inputfile", "core.project_base")
                    ...
                    shutil.copyfile(file_path, rewrite_filename)
