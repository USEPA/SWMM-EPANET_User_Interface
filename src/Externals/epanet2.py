...
# last modified:
#  - Feb 2011, David Hart, SNL

from Externals.epanet2toolkit import *
import ctypes, os, sys
from ctypes import byref, c_char_p, create_string_buffer, sizeof
...
class ENepanet():
    ...
    def __init__(self, inpfile='', rptfile='', binfile='', libepanet=''):
        ...
        if libepanet:
            try:
                self._set_lib(libepanet)
            except Exception as e1:
                print("Error loading specified library {0}:\n {1}\n".format(libepanet, str(e1)))

        if not self.ENlib:  # Did not set it from libfile specified as an argument, use a default path
            ...
            for lib in libnames:
                try:
                    if os.name in ['nt','dos']:
                        libepanet = resource_filename('pyepanet','data/Windows/%s.dll' % lib)
                    elif sys.platform in ['darwin']:
                        libepanet = resource_filename('pyepanet','data/Darwin/lib%s.dylib' % lib)
                    else:
                        libepanet = resource_filename('pyepanet','data/Linux/lib%s.so' % lib)
                    self._set_lib(libepanet)
                    return  # Successfully connected to C library
                except Exception as E1:
                    if lib == libnames[-1]:
                        raise E1
                    pass
    ...
    def ENsetnodevalue(self, iIndex, iCode, fValue):
        ...
        """
        self.errcode = self.ENlib.ENsetnodevalue(iIndex, iCode, fValue)
        self._error()
        return
    ...
    def ENsetlinkvalue(self, iIndex, iCode, fValue):
        ...
        """
        self.errcode = self.ENlib.ENsetlinkvalue(iIndex, iCode, fValue)
        self._error()
        return
    ...
    def ENsetpatternvalue(self, iIndex, iPeriod, fValue):
        ...
        """
        self.errcode = self.ENlib.ENsetpatternvalue(iIndex, iPeriod, fValue)
        self._error()
        return
    ...
    def ENsetoption(self, iCode, fValue):
        ...
        """
        self.errcode = self.ENlib.ENsetoption(iCode, fValue)
        self._error()
        return
